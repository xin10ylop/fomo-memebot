...
OUR_ADDRS = {a for a in (WALLET, RELAY) if a and a != "0x" + "0" * 40}   # 6.1: our wallet and relay; the shooters send to the relay, so a shot of ours is never counted as another sniper's
...
SHOOTER_KEYS = [k.strip() for k in os.environ.get("SHOOTER_KEYS", "").split(",") if k.strip()]   # 6.0: one wallet per shot (deploy/relay_ops.py shooters-create): the relay holds the
if SHOOTER_KEYS and not RELAY:                                             # stake and buys on any shooter's order, so no shot depends on another's nonce (Sep 19 08:53: "nonce too high" lost 29 of 35)
    raise SystemExit("SHOOTER_KEYS needs RELAY (the third relay, deploy/relay_deploy.py)")
try:
    SHOOTERS = [Account.from_key(k).address.lower() for k in SHOOTER_KEYS]
except Exception as e:
    raise SystemExit(f"SHOOTER_KEYS: a key is not a private key ({e})")
...
WALLET_STAKE = os.environ.get("WALLET_STAKE", "0" if RELAY else "1") == "1" and not SHOOTERS               # 5.93: live, every buy is sized to the wallet minus GAS_RESERVE_USD, so a second shot of the burst can never be
...
ATTACK_UNIT = os.environ.get("ATTACK_UNIT", "fleets").strip().lower()     # 6.3: what ATTACK_MIN counts. "fleets": distinct relay targets plus direct senders (6.1/6.2); "wallets": distinct
ATTACK_BUILD_MIN = int(os.environ.get("ATTACK_BUILD_MIN", "0"))           # shooter wallets behind any relay or firing directly, the count the tables score (crowd_signal.py). ATTACK_BUILD_MIN:
...
def load_send_step():
    ...
    if SHOOTERS and SEND_BURST is not None:
        ...
        if "keys" not in inspect.signature(SEND_BURST).parameters:
            raise SystemExit("SHOOTER_KEYS needs the send step that signs each shot with its shooter (keys=): sudo cp deploy/send_step.py /etc/sniper/send_step.py")
...
def score_flip(ts, wall, seen):
    ...
    try:
        ...
        state["slot_pred"][ts + 1] = (sp, vp)
        for k in [k for k in state["slot_pred"] if k < ts - 5]:
...
def seat_target(feed_ts, seconds):
    ...
    5.7 SLOT_SEND: the slot model's predicted arrival of the seat second's first block, less SLOT_LEAD_MS."""
...
GAS_SHOT_BEFORE, GAS_SHOT_FILL, GAS_SHOT_AFTER = 88_594, 137_693, 27_021   # 6.1: measured on the Sep 19 receipts (audit): a shot in the tax second (the curve reverts on minOut), the
                                                                             # filling shot through the relay, a shot after the fill (the relay refuses it, AlreadyBought)


def burst_gas_units(n=None):
    """gas used by a whole round trip: the burst's shots (about a third of them before the tick, measured 12 of 35), the approve and the
    sell; the plain buy when there is no burst. The old 230k assumed one buy: 8.4x low with the burst on (about 1.94M, $0.33 at $15)."""
    n = BURST_N if n is None else n
    if n > 1:
        before = round(n * 0.34)
        return before * GAS_SHOT_BEFORE + GAS_SHOT_FILL + max(0, n - 1 - before) * GAS_SHOT_AFTER + GAS_EST_APPROVE + GAS_EST_SELL
    return GAS_EST_BUY + GAS_EST_APPROVE + GAS_EST_SELL
...
def curve_buys(curve, since_ts=None):
    ...
    out.sort(key=lambda b: ((b[4] if len(b) > 4 and b[4] is not None else 0), b[3]))   # 6.5: chronological (block, arrival), not direct buys first: a stranger's
    if since_ts is not None:                                                            # reverting shot at block 2 folded before the helper's named buys at block 1
        out = [b for b in out if b[0] >= since_ts]                                      # closed the bundle at 0 (2 of 22 launches on Sep 25, one the day's sure fire)
    return out
...
def note_attack(w, to_hex, t, data, cv):
    """6.1: another sniper's shot at a watched curve, seen on the feed before the seat: a call to the curve itself from a wallet that is
...
def attack_wallets(w):
    """6.3: the distinct shooter wallets firing at this curve, behind any relay or directly: the count crowd_signal.py scores"""
    return len(w.get("attack_wallets", ()))


def attackers(w):
    """the count ATTACK_MIN is compared with, in the unit ATTACK_UNIT selects (report 24.33: the tables counted wallets, engine 6.1/6.2 fleets)"""
    return attack_wallets(w) if ATTACK_UNIT == "wallets" else attack_fleets(w)
...
# ------------------------------------------------------------------------------------------------------------ scoring
def block_clock(b_create):
    """a function block -> seconds since the creation block's clock second began, from the feed's flips (second -> its first
    block): blocks inside a second are spaced 0.1 apart, so the seat's second's first block sits at exactly 1.0 (E1) or 2.0
    (E2), as in the tables. None when the flips do not cover the launch (a restart within the last minute)."""
    fl = sorted((blk, ts) for ts, blk in state["flip_block"].items() if blk)
    if not fl or fl[0][0] > b_create or fl[-1][0] < b_create + 25:
        return None
    blocks = [x[0] for x in fl]
    import bisect
    def at(b):
        i = bisect.bisect_right(blocks, b) - 1
        blk, ts = fl[i]; return ts + (b - blk) / 10.0
    t0 = int(at(b_create))                                                  # the creation block's second
    return lambda b: at(b) - t0
...
def gas_usd():
    ...
    try:
        return min(5.0, max(0.05, (state.get("base_fee") or 2e8) * burst_gas_units() / 1e18 * state["eth_usd"]))
    except Exception:
        return 0.15


def exact_score(events, b_create, tk0, stake_eth, seat, tol=0.10, slip=0.3, hold=HOLD, frac=SUPPLY_FRAC, gated=True, stop_sell_frac=None, tp=None, front=False, readouts=True, t_entry=None, min_out=None, amount_in=None, info=None, tof=None):
    ...
    tof(block) -> seconds since the creation block's clock second began (6.06: the feed's flips give every block its second,
    so the seats sit on the real second boundaries as the tables do; without it, blocks after the creation at 9.9 a second,
    which put the seat's second up to 0.9 s late and read the front seat 9 points too low on Sep 19)."""
    rows = []; X, Y = X0, Y0; tier = None
    for i, (b, li, buy, q, tk, fee) in enumerate(events):
        t = tof(b) if tof is not None else (b - b_create) / 9.9
        if buy:
...
def score_launch(curve, tk0, b_create, stake_usd, creator, src, decision):
    ...
    try:
        ...
        info = {}; tof = block_clock(b_create)
        front_seat = SEAT == "E1" and BURST_N > 1                       # 6.05: the burst lands at the front of the seat's block (index 1-2 on every clean trade, Sep 19); the score, the
        r = exact_score(events, b_create, tk0, stake_usd / state["eth_usd"], SEAT, gated=BUNDLE_MIN > 0, stop_sell_frac=STOP_SELL_FRAC if STOP_SELL_FRAC > 0 else None, tp=TAKE_PROFIT if TAKE_PROFIT > 0 else None, info=info, front=front_seat, tof=tof)   # switch and the readouts follow that seat, not the tables' "0.3 s behind" (which read -8.5% while the front read the truth)
        if r is None:
        ...
        try:
            r1 = exact_score(events, b_create, tk0, stake_usd / state["eth_usd"], "E1", gated=True, front=True, readouts=False, tp=TAKE_PROFIT if TAKE_PROFIT > 0 else None, tof=tof)
        ...
        save_state()
        log({"ev": "score", "curve": curve, "roi": round(roi, 4), "roi_table": round(r[0] / r[1], 4), "roi_e1": roi_e1, "front_seat": front_seat, "clock": "seconds" if tof else "offset", "pnl_usd": round(pnl, 2), "cost_usd": round(cost, 2), "tier": round(tier, 4), "t_in_s": round(t_in, 2),
...
def burst_receipts(shots, timeout=10.0, settle_s=0.6):
    ...
    while mono() - t0 < timeout:
        if all(done):
            break
        if fill_at[0] is not None and mono() - fill_at[0] > settle_s:
            break
        time.sleep(0.02)
    stop.set(); state["t_fill"] = fill_at[0]                                   # 5.93: the pollers stop here (a shot the sequencer dropped was polled for the full timeout), and the
...
def wallet_reserve_eth():
    """what the wallet must keep out of the stake: the burst's reverted shots, the approve and the exit's fee ceiling, plus the upfront
    gas the sequencer checks on the filling shot (limit x price, refunded after execution); measured shots cost about a cent"""
    gp = state.get("gas_price") or 2e8; px = max(state["eth_usd"], 1.0)
    upfront = GAS_BUY * gp / 1e18; shots = BURST_N * GAS_EST_BUY * (state.get("base_fee") or gp / GAS_HEADROOM) / 1e18
    return max(GAS_RESERVE_USD / px, shots + upfront + (SELL_FEE_MAX_USD + 0.10) / px)


def refresh_wallet(why=""):
    """live: read the wallet now (after a landing or an exit) so the next stake is sized on what is really there"""
    if SEND is None:
        return
    try:
...
def kill_gate():
    """6.1: the dollar line set before the start: below it no launch is taken and one alarm is logged (a change of KILL_USD or a
    top-up clears it)"""
    b = state.get("bankroll")
    if KILL_USD > 0 and b is not None and b < KILL_USD:
        if not state.get("killed"):
            state["killed"] = True; log({"ev": "alarm", "what": f"kill line: capital ${b:.2f} (wallet plus relay) is below ${KILL_USD:.2f}; no launch is taken until it is above"})
        return f"kill line: capital ${b:.2f} < ${KILL_USD:.2f}"
    state["killed"] = False
    return None
...
def refresh_shooters(nonces=True, gas=True):
    ...
    for a in SHOOTERS:
        try:
            if nonces:
                state["shooter_nonce"][a] = int(rpc.call("eth_getTransactionCount", [a, "pending"]), 16)
            if gas:
                state["shooter_eth"][a] = int(rpc.call("eth_getBalance", [a, "latest"]), 16) / 1e18
        except Exception as e:
...
def _handle_creation(creator, quote, init_buy_wei, seen_at, feed_ts, named, blk0, tax_bps=None, txh=None, known=None):
    ...
    with lock:
        ...
        stake_usd = min(STAKE_MAX, max(STAKE_MIN, state["bankroll"] * FRAC)); gates = []; reserve_eth = 0.0
        if WALLET_STAKE and SEND is not None:                             # 5.93: the stake is the wallet less the gas reserve, so a second fill cannot be funded
            reserve_eth = wallet_reserve_eth(); we = state.get("wallet_eth")
            if we is None or mono() - state.get("wallet_at", -1e9) > WALLET_MAX_AGE_S:
                gates.append("wallet balance not read in the last minute (RPC): the stake cannot be sized")
            else:
                stake_usd = max(0.0, we - reserve_eth) * state["eth_usd"]
                if stake_usd < STAKE_MIN:
                    gates.append(f"wallet ${we * state['eth_usd']:.2f} less the ${reserve_eth * state['eth_usd']:.2f} gas reserve is below the minimum stake ${STAKE_MIN:.0f}: top up")
                if stake_usd > STAKE_MAX * 1.05:
                    gates.append(f"wallet ${we * state['eth_usd']:.2f} holds more than STAKE_MAX ${STAKE_MAX:.0f}: a burst could fill twice at a smaller stake; withdraw the excess or raise STAKE_MAX")
                if stake_usd <= 2 * reserve_eth * state["eth_usd"]:
                    gates.append("stake below twice the gas reserve: a second shot could be funded")
        ...
        if not gates:
            state["busy_until"] = mono() + HOLD_EFF + 3; nonce = state["nonce"]; state["nonce"] += (2 if SHOOTERS else BURST_N + 2); gas_price = state["gas_price"]   # the shots (unless the shooters fire them), the approve, the sell
            shooters_now = list(ready[:BURST_N]) if SHOOTERS else []
    if gates:
    ...
    if burst_at is not None:                                          # 5.92: size and build as late as the signing allows, on the freshest reserves.
        t_build = burst_at - (0.0015 * BURST_N + 0.03)                    # Built half a second early, the minOut missed the bundle's last buys of the
...
def prune(now):
    for k in ("flip_at", "flip_block", "flip_wall"):
        d = state[k]
        if len(d) > 4000:                                                  # keep about an hour of seconds
            for ts in sorted(d)[:-3600]:
                d.pop(ts, None)
    ...
    for ts_ in [k for k in state["flip_at"] if k < state["feed_ts"] - 120]:
        state["flip_at"].pop(ts_, None); state["flip_block"].pop(ts_, None); state["flip_wall"].pop(ts_, None)
            ...
            if watched:                                                              # 6.1: every call that touches a watched curve counts as a sniper's shot (see note_attack)
                for cv_, w_ in list(watched.items()):
                    if to_hex == cv_ or w_["cb"] in data:
                        note_attack(w_, to_hex, t, data, cv_)
...
async def main():
    ...
    if RELAY and SEND is not None:                                        # 5.94/6.03: after the send step is loaded (before it, SEND was None and none of this ran); a wrong relay address would burn every shot; the contract must be there and be ours
        ...
        if SHOOTERS:
            if BURST_N > len(SHOOTERS):
                raise SystemExit(f"BURST_N={BURST_N} but only {len(SHOOTERS)} shooters: one shooter per shot (deploy/relay_ops.py shooters-create {BURST_N})")
            try:
                rpc.call("eth_call", [{"to": RELAY, "data": "0x5c7b6bcb" + abi_word(WALLET)}, "latest"])
            except Exception as e:
                raise SystemExit(f"RELAY {RELAY} does not know shooters (an older relay): redeploy with deploy/relay_deploy.py --write-env, then deploy/relay_ops.py shooters-register")
            missing = [a for a in SHOOTERS if not int(rpc.call("eth_call", [{"to": RELAY, "data": "0x5c7b6bcb" + abi_word(a)}, "latest"]) or "0x0", 16)]
            if missing:
                raise SystemExit(f"{len(missing)} of {len(SHOOTERS)} shooters are not registered on the relay: deploy/relay_ops.py shooters-register")
            refresh_shooters()
            log({"ev": "shooters", "n": len(SHOOTERS), "addresses": list(SHOOTERS), "gas_eth": {a[:10]: round(state["shooter_eth"].get(a, 0), 6) for a in SHOOTERS}, "low": sum(1 for a in SHOOTERS if state["shooter_eth"].get(a, 0) < SHOOTER_MIN_ETH)})
        try:                                                                  # a buy with a deadline in the past must answer TooLate: an older relay (no deadline) answers with a plain revert
            rpc.call("eth_call", [{"from": WALLET, "to": RELAY, "data": "0x" + RELAY_BUY_SEL + abi_word(RELAY) + abi_word(10 ** 12) + abi_word(0) + abi_word(1)}, "latest"], tries=1)   # no value: the third relay is not payable
