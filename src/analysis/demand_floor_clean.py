...
for k, recs in by_win.items():
    for i, r in enumerate(recs):
        prev = [y["feth"] for y in recs[:i] if y["kept"] and y["t_score"] <= r["t"]][-60:]
        r["gauge"] = st.mean(prev) if len(prev) >= 20 else None
        if r["kept"] and r["gauge"] is not None:
            for lo, hi in bins:
                if lo <= r["gauge"] < hi: tab[(lo, hi)].append(r["roi"])
print("kept trades by the 60-launch demand reading at decision time (needs 20 scored CLEAN launches, the engine's own definition), all windows:")
for b in bins:
