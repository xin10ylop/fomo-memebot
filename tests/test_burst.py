...
if calls:
    ...
    check("burst: built late on fresh reserves, 25-60 ms before the first shot (5.92), and handed to the send step then", d[0].get("send_mode") == "predict-prebuilt" and 25 <= (d[0].get("prebuilt_ms") or 0) <= 60 and 25 <= (d[0].get("build_lead_ms") or 0) <= 60, f"{d[0].get('send_mode')} prebuilt {d[0].get('prebuilt_ms')} build_lead {d[0].get('build_lead_ms')}")
