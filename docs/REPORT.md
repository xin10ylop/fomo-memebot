...
11. **Round 5 priced the seat on the other side of every losing scalp: the creator's.** On the Pons V2 curve the creator's launch-block buy is the only trade with no competition and no snipe tax, and selling walks the same bonding curve back, so the exit proceeds are exact. Replaying 6,108 launches with their actual sale timing: creators sell their launch-block layer within 13 seconds (median) on 97% of launches, realize +10% to +17% of stake on the sale plus 5–8% of stake in trading fees, with a floor of −1% when nobody buys (the curve refunds the stake minus the fee). Fit hours +10%, holdout hours +22% of stake per launch for serial launchers; +69% / +137% for one-off creators with small stakes; 99% of launches end positive; the money comes from the sniper bots and scalpers whose rules section 12 shows losing 20–46% per trade. It is the only directional mechanism in this repository with measured positive expectancy in both halves of the sample, it needs no price prediction, and it is open to anyone who runs a launch bot. It is also the reason the chain sees 28,000 launches a day, it is capacity-limited by how much the bots keep buying, and its edge ends the day the snipers stop. Section 13.
...
## 13. Round 5: the creator's seat, priced exactly

Every filter and scalp in section 12 loses. Someone is on the other side of those trades in the first minute of a launch. Section 11.6 measured the creator seat only as a fee base rate ($42 per launch on average). This round prices it as a strategy, using the mechanics of the Pons V2 curve.

**Mechanics that make it computable.** The creator's initial buy is part of the creation transaction and is the only trade that executes in the launch block (no latency race, no 99%-decaying snipe tax, which applies to buys in the next five seconds). All supply sits on a deterministic bonding curve; selling walks the same curve backwards, so the quote a creator can take out for their tokens at any moment equals what the most recent buyers paid for the top slice of sold supply, minus the 1% fee. `src/analysis/creator_seat.py` replays that exactly for the 6,108 launches of 12:00–18:00 UTC on Sep 3, detects the creator's sale as the first sell that eats into the launch-block layer (only the holder of that layer can do it), and adds 0.7% of all curve volume as the creator's fee share.

**Results (per launch, as % of the creator's launch-block stake; fit = 12:00–15:00, holdout = 15:00–18:00)**

| Creator class | n | stake / launch (quote units) | fees | sale, as actually timed | fees + sale, fit | fees + sale, holdout | launches ending positive | median time to sale |
|---|---|---|---|---|---|---|---|---|
| serial (≥10 launches/day) | 3,074 | 6.8 | +5.7% | +12% | **+10%** | **+22%** | 99% | 13 s (p90 54 s) |
| 2–9 launches/day | 1,147 | 1.5 | +17% | +11% | +32% | +25% | 99–100% | < 1 min |
| one-off | 1,887 | 0.34 | +51% | +56% | +69% | +137% | 99–100% | 1 min |
| all | 6,108 | 3.8 | +8% | +13% | +14% | +25% | 99% | |

Distribution for serial launchers, realized sale only: p10 −1%, p25 −1%, median −1%, p75 +2%, p90 +8%; no launch below −5%. The floor is the curve refunding the stake minus fee when nobody buys; the upside is whatever the sniper bots pay in the first seconds; the fee share is the annuity on top. Best-time exits would have been +41% to +173% of stake, so creators leave money on the table by dumping at once, and that is rational: it takes the bots' first buys and needs no read on whether the token will run (serial creators' tokens double after the first minute only 1.8% of the time, section 12).

**Who the busiest launchers are**: 0x7b444d22 (158 launches in six hours, fees + sale +25% of stake), 0xa60e892a (116, +14%; the same wallet deployed 54 of the tokens the leaderboard traded), 0x1d0e439a (71, +13%), 0x7de5b9c8 (158, +2%; the deployer of 31 leaderboard-traded tokens). Their launches are named after whatever is trending, and their counterparties are the auto-buy bots and the scalp rules of section 12.

**What it is, honestly**: this is the one directional mechanism in the repository with positive measured expectancy on both halves of the sample, a floor near zero per launch, turnover of minutes, and a fee annuity attached. It is not prediction; it is holding the seat that every sniper pays. It is executable by a retail account with a launch bot (creation transaction with the initial buy, one sell transaction ten seconds later, $1 launch fee, gas sponsored until early October and pennies after), and its capacity is bounded by how much the bots keep buying: the 28,000 launches a day are the market clearing that seat. It is also the spam that the chain's own numbers (0.077% of launches ever tracked) describe, and it stops the day the bots stop.

**Forward test (frozen)**: a launch bot with a fixed stake of 0.02–0.05 ETH, a 1-second-decaying-tax-safe sell at 10 seconds or at 1.5× curve proceeds, 50 launches, names drawn from the day's trending list; expected +15–20% of stake per launch on the Sep 3 numbers, with a hard stop if the first 20 launches average below +5%. The live measurement that needs no capital first: run `creator_seat.py` on a new day's pull to confirm the seat still pays before launching anything.
